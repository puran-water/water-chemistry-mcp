...
class DatabaseManager:
    ...
    def get_database_info(self, database_path: str) -> Dict:
        ...
        is_usgs = database_path.startswith(USGS_PHREEQC_DATABASE_PATH)
    ...
    def get_compatible_minerals(self, database_path: str, requested_minerals: Optional[List[str]] = None) -> Union[List[str], Dict[str, str]]:
        """
                ...
                if mineral in compatible_minerals:
                        ...
                        for db_mineral, info in DATABASE_SPECIFIC_MINERALS[db_name].items():
                            if mineral in info['alternative_names']:
                                ...
                                if db_mineral in compatible_minerals:
                                    ...
                                    logger.info(f"Substituting mineral '{mineral}' with '{db_mineral}' for database {db_name}")
                                    break
                        ...
                        for common_mineral, info in COMMON_MINERALS.items():
                            if mineral in info['alternative_names']:
                                ...
                                if common_mineral in compatible_minerals:
                                    ...
                                    found_alternative = True
                                    logger.info(f"Substituting mineral '{mineral}' with common mineral '{common_mineral}'")
                                    break
                    
                    # Special case handling for common problematic minerals
                    if not found_alternative:
                        if mineral == 'Ferrihydrite' and db_name == 'phreeqc.dat' and 'Fe(OH)3(a)' in compatible_minerals:
                            mineral_mapping[mineral] = 'Fe(OH)3(a)'
                            found_alternative = True
                            logger.info(f"Special case: Substituting '{mineral}' with 'Fe(OH)3(a)' for database {db_name}")
                        ...
                        if mineral_formula:
                            ...
                            for db_mineral, db_info in DATABASE_SPECIFIC_MINERALS.get(db_name, {}).items():
                                if db_info['formula'] == mineral_formula and db_mineral in compatible_minerals:
                                    mineral_mapping[mineral] = db_mineral
                                    found_alternative = True
                                    logger.info(f"Substituting mineral '{mineral}' with '{db_mineral}' based on formula match ({mineral_formula})")
                                    break
                    ...
                    if not found_alternative:
                        for other_db, minerals in DATABASE_SPECIFIC_MINERALS.items():
                            ...
                            for other_mineral, info in minerals.items():
                                if mineral in info['alternative_names'] or info['formula'] == self._get_mineral_formula(mineral):
                                    ...
                                    for db_mineral, db_info in DATABASE_SPECIFIC_MINERALS.get(db_name, {}).items():
                                        if db_info['formula'] == formula and db_mineral in compatible_minerals:
                                            mineral_mapping[mineral] = db_mineral
                                            found_alternative = True
                                            logger.info(f"Substituting mineral '{mineral}' with '{db_mineral}' based on formula match")
                                            break
                    ...
                    if not found_alternative:
                        ...
                        if closest_match:
                            ...
                            logger.warning(f"Using best guess: Substituting '{mineral}' with '{closest_match}' for database {db_name}")
                        else:
                            # No compatible alternative found
                            mineral_mapping[mineral] = None
                            logger.warning(f"No compatible substitute found for mineral '{mineral}' in database '{db_name}'")
    ...
    def register_custom_database(self, database_path: str, name: Optional[str] = None, 
