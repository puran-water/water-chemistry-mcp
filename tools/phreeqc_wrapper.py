...
async def run_phreeqc_simulation_with_precipitation(
    input_string: str, database_path: Optional[str] = None, remove_precipitates: bool = True, num_steps: int = 1
) -> Union[Dict[str, Any], List[Dict[str, Any]]]:
    """
    ...
    try:
        ...
        if "precipitated_phases" in results:
            ...
            mineral_mw = {
                ...
                "Fluorite": 78.07,
                "CaF2": 78.07,
                "Hydroxyapatite": 502.31,
                "Ca5(PO4)3(OH)": 502.31,
                "Strengite": 150.82,
                "FePO4": 150.82,
                "Struvite": 245.41,
                "MgNH4PO4:6H2O": 245.41,
            }
    ...
    for mineral in minerals:
        ...
        try:
            ...
            for time, sol in solution.kinetics(
                formula=mineral, rate_function=rate_func, time=time_steps, m0=0  # Starting with no precipitated solid
            ):
        ...
        for mineral in minerals_kinetic.keys():
            ...
            if isinstance(raw_results, list):
                ...
                if raw_results:
                    ...
                    results["final_solution"] = last_result.get(
                        "solution_summary", {}
                    )  # Create kinetic profile for this mineral
            if time_points:
                profile = KineticPrecipitationProfile(
                    mineral=mineral,
                    time_seconds=time_points,
...
async def query_database(query_term: str, query_type: str, database_path: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if query_type.lower() == "keyword_block":
            ...
            if has_phases_block:
                ...
                phase_block_match = re.search(
                    r"^PHASES\b(.*?)(?=^[A-Z_]+\b)", content, re.MULTILINE | re.IGNORECASE | re.DOTALL
                )
                if not phase_block_match:
                    # Try alternative pattern if the first one fails
                    phase_block_match = re.search(
                        r"PHASES(.*?)(?=^[A-Z_]+\b|\Z)", content, re.MULTILINE | re.IGNORECASE | re.DOTALL
                    )
                ...
                if phase_block_match:
                    ...
                    if not phase_match:
                        ...
                        if phase_match:
                            ...
                            next_mineral_match = re.search(
                                r"^\s*[A-Za-z0-9_\-]+\s+", phase_block[start_idx + 1 :], re.MULTILINE
                            )
                    ...
                    if not results:
                        ...
                        if alt_names:
                            output["error"] = (
                                f"Mineral/Phase '{query_term}' not found within PHASES block. Try alternative names: {', '.join(alt_names[:3])}"
                            )
                        ...
                        if re.search(r"=\s+[A-Za-z0-9]+[+\-0-9]*\s+\+", line):
                            ...
                            for j in range(1, 5):  # Get up to 4 more lines
                                if i + j < len(content.splitlines()):
                                    ...
                                    if not next_line.strip().startswith("#") and not re.match(
                                        r"^\s*[A-Za-z0-9_\-]+\s+=", next_line
                                    ):
                ...
                if direct_matches:
                    ...
                    output["error"] = (
                        f"PHASES block not found and mineral '{query_term}' not found directly in database content."
                    )

        elif query_type.lower() == "species":
            # Find species in SOLUTION_SPECIES block
            species_block_match = re.search(
                r"^SOLUTION_SPECIES\b(.*?)(?=^[A-Z_]+\b|\Z)", content, re.MULTILINE | re.IGNORECASE | re.DOTALL
            )
            if species_block_match:
                ...
                species_match = re.search(
                    f"^\\s*{re.escape(query_term)}\\b.*?(?=^\\s*[A-Za-z0-9_]+\\s+|$)",
                    species_block,
            ...
            master_block_match = re.search(
                r"^SOLUTION_MASTER_SPECIES\b(.*?)(?=^[A-Z_]+\b|\Z)", content, re.MULTILINE | re.IGNORECASE | re.DOTALL
            )
            if not master_block_match:
                # Try alternative pattern if the first one fails
                master_block_match = re.search(
                    r"SOLUTION_MASTER_SPECIES(.*?)(?=^[A-Z_]+\b|\Z)", content, re.MULTILINE | re.IGNORECASE | re.DOTALL
                )
            ...
            if master_block_match:
                ...
                # Example: "Ca" or "Fe(2)"
                element_exact_match = re.search(
                    f"^\\s*{re.escape(query_term)}\\s+.*", master_block, re.MULTILINE | re.IGNORECASE
                )
                ...
                if query_base != query_term:
                    element_base_match = re.search(
                        f"^\\s*{re.escape(query_base)}\\s+.*", master_block, re.MULTILINE | re.IGNORECASE
                    )
                ...
                for line in master_block.splitlines():
                    ...
                    if len(line_words) > 0:
                        ...
                        if (
                            first_word == query_lower  # Exact match: "Ca" matches "Ca"
                            or (
                                query_lower != query_base_lower and first_word_base == query_base_lower
                            )  # Base match: "Fe" matches "Fe(2)"
                            or (query_lower == query_base_lower and query_lower in first_word_base)
                        ):  # Partial match: "Fe" is in "Fe(2)"
                ...
                if elements_found:
                    ...
                    for name, symbol in element_map.items():
                        if query_lower == symbol.lower():
                            ...
                            element_match = re.search(
                                f"^\\s*{re.escape(symbol)}\\b.*", master_block, re.MULTILINE | re.IGNORECASE
                            )
                            if element_match:
                    ...
                    if not results and suggestions:
                        output["error"] = (
                            f"Element '{query_term}' not found within SOLUTION_MASTER_SPECIES block. Try alternative names: {', '.join(suggestions)}"
                        )
                ...
                if element_lines:
                    ...
                    output["error"] = (
                        f"SOLUTION_MASTER_SPECIES block not found and element '{query_term}' not found in database content."
                    )
        ...
        output["context"] = {"query_term": query_term, "query_type": query_type, "exception": str(e)}
