...
def calculate_mass_balance(
    ...
    # Overall assessment
    unbalanced_elements = [e for e, data in mass_balance.items() 
                          if not data["balanced"] and data["total_initial"] > 0.001]
    
    mass_balance["summary"] = {
        "all_balanced": len(unbalanced_elements) == 0,
        "unbalanced_elements": unbalanced_elements,
        "max_error_percent": max([data["balance_percent"] for data in mass_balance.values() 
                                 if isinstance(data, dict) and "balance_percent" in data], default=0)
    }
