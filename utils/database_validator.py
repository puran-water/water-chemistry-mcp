...
def extract_database_elements(database_path: str) -> Dict[str, Dict[str, Any]]:
    ...
    try:
        ...
        master_species_match = re.search(r'SOLUTION_MASTER_SPECIES\s+(.*?)(?=^\w+|\Z)', 
...
def extract_database_minerals(database_path: str) -> Dict[str, Dict[str, Any]]:
    ...
    try:
        ...
        phases_match = re.search(r'PHASES\s+(.*?)(?=^\w+|\Z)', 
                               content, re.MULTILINE | re.DOTALL)
        ...
        if phases_match:
            ...
            mineral_matches = re.findall(r'^\s*([A-Za-z0-9_().-]+)\s+$(.*?)(?=^\s*[A-Za-z0-9_().-]+\s*$|\Z)', 
                                        phases_block, re.MULTILINE | re.DOTALL)
            ...
            for name, definition in mineral_matches:
                ...
                analytic_match = re.search(r'(-a_e|a_e)\s+([0-9.-]+)\s+([0-9.-]+)\s+([0-9.-]+)\s+([0-9.-]+)\s+([0-9.-]+)', definition, re.IGNORECASE)
                analytic = list(map(float, analytic_match.groups()[1:])) if analytic_match else None
                
                # Extract reaction
                reaction_lines = [line.strip() for line in definition.split('\n') 
...
def extract_solution_species(database_path: str) -> Dict[str, Dict[str, Any]]:
    ...
    try:
        ...
        species_match = re.search(r'SOLUTION_SPECIES\s+(.*?)(?=^\w+|\Z)', 
                                content, re.MULTILINE | re.DOTALL)
        ...
        if species_match:
            ...
            species_matches = re.findall(r'^\s*([^=\n#]+?)(?:\s*=\s*([^\n#]*))?\s*$\s*(.*?)(?=^\s*[^=\n#]+(?:\s*=|$)|\Z)', 
                                         species_block, re.MULTILINE | re.DOTALL)
...
def compare_databases(db_path1: str, db_path2: str) -> Dict[str, Any]:
    ...
    # Compare statistics
    results['statistics'] = {
        'db1': validation1['statistics'],
        'db2': validation2['statistics']
    }
...
def verify_mineral_compatibility(database_path: str, mineral_names: List[str]) -> Dict[str, Any]:
    ...
    try:
        ...
        for mineral in mineral_names:
            ...
            if mineral.lower() in db_minerals_lower:
                results['compatible'].append({
                    'requested': mineral,
                    'database': db_minerals_lower[mineral.lower()]
                })
                continue
            ...
            for db_mineral_lower, db_mineral in db_minerals_lower.items():
                ...
                if (mineral.lower() in db_mineral_lower or
                    db_mineral_lower in mineral.lower() or
                    ...
                    results['similar'].append({
                        ...
                        'confidence': 'medium' if mineral.lower() in db_mineral_lower or db_mineral_lower in mineral.lower() else 'low'
                    })
                    found_similar = True
                    break
        ...
        results['summary'] = {
            ...
            'compatibility_percentage': round(100 * len(results['compatible']) / len(mineral_names)) if mineral_names else 0
        }
