...
class WaterAnalysisInput(BaseModel):
    ...
    units: Optional[str] = Field("mg/L", description="Units for concentration values (e.g., mg/L, mmol/L, ppm). Default mg/L.")
    charge_balance: Optional[str] = Field(None, description="Element/species to adjust for charge balance (e.g., 'Na' or 'Cl'). pH is ignored if this is set.")
    density: Optional[float] = Field(None, description="Solution density in kg/L (or g/cm3).")
    redox: Optional[str] = Field(None, description="Redox couple to define pe (e.g., 'O(-2)/O(0)' or 'Fe(2)/Fe(3)'. Overrides pe if set.")
...
class SolutionOutput(BaseModel):
    ...
    solution_summary: Optional[Dict[str, Any]] = Field(None, description="Summary properties like pH, pe, alkalinity, TDS, density.")
    saturation_indices: Optional[Dict[str, float]] = Field(None, description="Calculated saturation indices for relevant minerals.")
    phases: Optional[List[Dict[str, Any]]] = Field(None, description="Information on phases at equilibrium (precipitated/dissolved amounts).")
    ...
    gas_phase_composition: Optional[Dict[str, Any]] = Field(None, description="Composition of the equilibrium gas phase (partial pressures, moles).")
    ...
    precipitated_phases: Optional[Dict[str, float]] = Field(None, description="Minerals that precipitated and their amounts in moles.")
    precipitation_occurred: Optional[bool] = Field(None, description="Whether any precipitation occurred.")
    precipitation_estimated: Optional[bool] = Field(None, description="Whether precipitation amounts are estimated rather than calculated.")
    total_precipitate_g_L: Optional[float] = Field(None, description="Total mass of precipitated solids in g/L.")
    precipitate_details: Optional[Dict[str, Dict[str, float]]] = Field(None, description="Detailed information about each precipitate including moles, mass, and MW.")
...
class ReactantInput(BaseModel):
    ...
    units: Optional[str] = Field("mmol", description="Units for the amount (e.g., 'mmol', 'g', 'mg'). Assumed per liter unless specified otherwise.")
...
class CustomKineticMineralParameters(BaseModel):
    ...
    activation_energy: Optional[float] = Field(48000, description="Activation energy in J/mol for temperature correction.")
    surface_area_exponent: Optional[float] = Field(0.67, description="Exponent for surface area evolution (typically 0.67 for spheres).")
...
class SimulateChemicalAdditionInput(BaseModel):
    ...
    pressure_atm: Optional[float] = Field(None, description="Override pressure for reaction step (defaults to initial_solution press).")
...
class KineticPrecipitationProfile(BaseModel):
    ...
    precipitation_rate_mol_s: List[float] = Field(..., description="Instantaneous precipitation rate at each time point.")

class SimulateChemicalAdditionOutput(SolutionOutput):
    kinetic_profiles: Optional[List[KineticPrecipitationProfile]] = Field(None, description="Time-series precipitation data when kinetic modeling is used.")
    kinetic_modeling_used: Optional[bool] = Field(False, description="Whether kinetic modeling was used instead of equilibrium.")

# Tool 3: calculate_dosing_requirement
class TargetConditionInput(BaseModel):
    parameter: str = Field(..., description="Parameter to target (e.g., 'pH', 'SI', 'Alkalinity', 'pe', 'Concentration').")
    ...
    element_or_species: Optional[str] = Field(None, description="Element or species name if parameter is 'Concentration'.")
    units: Optional[str] = Field(None, description="Units for target value if parameter is 'Alkalinity' or 'Concentration' (e.g., 'mmol/kgw', 'mg/L').")
...
class CalculateDosingRequirementInput(BaseModel):
    ...
    tolerance: Optional[float] = Field(0.05, description="Acceptable tolerance for reaching the target (e.g., pH units, SI units). Default 0.05.")
...
class CalculateDosingRequirementOutput(BaseModel):
    ...
    final_state: SolutionOutput = Field(..., description="The calculated solution state after adding the required dose.")
    iterations_taken: Optional[int] = Field(None, description="Number of iterations performed.")
    convergence_status: str = Field(..., description="Status message (e.g., 'Converged', 'Max iterations reached', 'Error').")
    error: Optional[str] = Field(None, description="Error message if calculation failed.")
...
class PredictScalingPotentialInput(WaterAnalysisInput):
    force_equilibrium_minerals: Optional[List[str]] = Field(None, description="Optional list of mineral names to force equilibrium with (calculates mass precipitated).")
    database: Optional[str] = Field(None, description="Path or name of the PHREEQC database file to use.")
...
class GasPhaseDefinition(BaseModel):
    type: str = Field("fixed_pressure", description="Type of gas phase ('fixed_pressure' or 'fixed_volume'). Default 'fixed_pressure'.")
    initial_components: Dict[str, float] = Field(..., description="Dictionary of gas components and their initial partial pressures (atm) for fixed_pressure, or moles for fixed_volume.")
    fixed_pressure_atm: Optional[float] = Field(1.0, description="Total pressure for fixed_pressure type. Default 1.0.")
    initial_volume_liters: Optional[float] = Field(1.0, description="Initial volume for fixed_volume type or initial bubble size for fixed_pressure. Default 1.0.")
...
class TargetRedoxCondition(BaseModel):
    parameter: str = Field(..., description="How to define the target redox state ('pe', 'Eh_mV', 'equilibrate_with_couple').")
    value: Optional[float] = Field(None, description="Target value for 'pe' or 'Eh_mV'.")
    couple_name: Optional[str] = Field(None, description="Name of the redox couple (e.g., 'O2(g)', 'Fe(+3)/Fe(+2)') if parameter is 'equilibrate_with_couple'.")
    couple_logK_or_pressure: Optional[float] = Field(None, description="LogK or partial pressure (atm) for the equilibrium couple.")
...
class SurfaceDefinition(BaseModel):
    ...
    sites_block_string: Optional[str] = Field(None, description="Raw PHREEQC SURFACE_MASTER_SPECIES and SURFACE_SPECIES blocks if not in main DB.")
    sites_info: Optional[List[Dict[str, Any]]] = Field(None, description="Structured list of sites (e.g., [{'name': 'Hfo_s', 'moles': 0.1, 'specific_area_m2_g': 600, 'mass_g': 0.5}]).")
...
class SimulateSurfaceInteractionInput(BaseModel):
    ...
    database: Optional[str] = Field(None, description="Path or name of the PHREEQC database file to use (must contain surface species definitions).")
...
class KineticReaction(BaseModel):
    ...
    name: str = Field(..., description="Reaction name that matches a RATES block (e.g., 'Calcite', 'Pyrite_oxidation').")
    formula: Optional[Union[str, Dict[str, Union[float, int]]]] = Field(None, description="Optional chemical formula (e.g., 'CaCO3', 'FeS2') or formula dictionary (e.g., {'N(5)': -1, 'N(3)': 1}).")
    parameters: Optional[Dict[str, Union[float, str]]] = Field(None, description="Rate parameters (e.g., {m: 1, m0: 1, parm(1): 0.1}).")
...
class QueryThermoDatabaseInput(BaseModel):
    query_term: str = Field(..., description="Element, species formula, mineral name, or keyword (e.g., 'Ca', 'HCO3-', 'Calcite', 'SOLUTION_SPECIES').")
    query_type: str = Field(..., description="Type of query ('species', 'mineral', 'element_info', 'keyword_block').")
    database: Optional[str] = Field(None, description="Path or name of the PHREEQC database file to query. Default to phreeqpython default.")

class QueryThermoDatabaseOutput(BaseModel):
    query_term: str = Field(...)
    query_type: str = Field(...)
    database_used: str = Field(...)
    results: Optional[Union[Dict[str, Any], List[str], str]] = Field(None, description="Found database information (e.g., reaction, log_K, raw block text). Structure depends on query type.")
...
class GenerateCalculationSheetInput(BaseModel):
    ...
    calculation_type: str = Field(
        ...
        examples=["lime_softening", "pH_adjustment", "phosphate_removal", "metal_precipitation", "scaling_assessment", "kinetic_design", "treatment_train"]
    )
    ...
    include_raw_outputs: bool = Field(
        True, 
        description="Whether to include raw PHREEQC outputs in appendices."
    )
    output_formats: List[str] = Field(
        ["html", "pdf"], 
        description="Desired output formats for the calculation sheet."
    )
