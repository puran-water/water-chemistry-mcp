...
SULFIDE_MINERALS = ["Pyrite", "Sphalerite", "Galena", "Cinnabar", "Chalcocite"]  # FeS2  # ZnS  # PbS  # HgS  # Cu2S
...
METAL_REMOVAL_MINERALS_BY_DB = {
    "phreeqc.dat": ["Fe(OH)3(a)", "Goethite", "Hematite", "Al(OH)3(a)", "Boehmite", "Zn(OH)2(a)", "Cu(OH)2"],
    "wateq4f.dat": [
        "Ferrihydrite",
        "Goethite",
        "Hematite",
    ...
    "llnl.dat": ["Ferrihydrite", "Goethite", "Hematite", "Gibbsite", "Boehmite", "Zn(OH)2(am)", "Cu(OH)2", "Tenorite"],
}
                            ...
                            if mineral not in minerals:
                                if database_validator_available():
                                    if mineral_exists_in_database(database_path, mineral):
                                        ...
                                        logger.warning(
                                            f"Metal removal mineral {mineral} defined in registry but not found in database {db_name}"
                                        )
                                else:
                                    minerals.append(mineral)
                    except Exception as e:
                            ...
                            if database_validator_available():
                                if mineral_exists_in_database(database_path, mineral):
                                    ...
                                    if mineral == "Ferrihydrite" and db_name == "phreeqc.dat":
                                        ...
                                        if "Fe(OH)3(a)" not in minerals and mineral_exists_in_database(
                                            database_path, "Fe(OH)3(a)"
                                        ):
                            ...
                            if database_validator_available():
                                if mineral_exists_in_database(database_path, mineral):
                                    ...
                                    if mineral == "Gibbsite" and db_name == "phreeqc.dat":
                                        ...
                                        if "Al(OH)3(a)" not in minerals and mineral_exists_in_database(
                                            database_path, "Al(OH)3(a)"
                                        ):
